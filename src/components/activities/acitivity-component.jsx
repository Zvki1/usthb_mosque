  ...
  return (
    ...
    md:hover:scale-105 transition-transform duration-500 ease-in-out  flex-1 min-h-full min-w-full ">
      ...
      <div className="bg-white dark:bg-gray-950 p-4 md:p-6 space-y-3 text-right">
        <h3 className="text-2xl font-Tajawal text-[#243245] font-semibold">{data?.title || "بطاقة العنوان"}</h3>
        <p className="text-[#243245] font-medium font-Tajawal line-clamp-3">
          {data?.paragraph || "بطاقة النص"}
        </p>
        <Link className="flex justify-center  px-14" to={`/details/${data?._id}`}
        state={{ data }}
        >
          <Button
